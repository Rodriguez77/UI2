...
import com.example.ui2.Album
import com.example.ui2.AlbumSongs
import com.example.ui2.Song
...
class DatabaseHelper(var context: Context): SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    //album song
    fun albumSongCreate(albumSong: AlbumSongs): Boolean {
    ...
    fun albumSongUpdate(albumSong: AlbumSongs): Boolean {
        val database = this.writableDatabase
        val contentValues = ContentValues()
        contentValues.put(ALBUM_SONG_TITLE, albumSong.title)
        contentValues.put(ALBUM_SONG_ARTIST, albumSong.artist)
    ...
    fun albumSongDelete(albumSong: AlbumSongs): Boolean {
        val database = this.writableDatabase
    ...
    fun albumSongRead(): MutableList<AlbumSongs> {
        val albumList: MutableList<AlbumSongs> = ArrayList<AlbumSongs>()
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                val album = AlbumSongs(id, title, artist)
                albumList.add(album)
            } while (cursor.moveToNext())
        }
        return albumList
    ...
    fun albumSongReadOne(album_id: Int): AlbumSongs {
        var albumOne: AlbumSongs = AlbumSongs(0, "", "")
        ...
        if (cursor.moveToFirst()) {
            ...
            albumOne = AlbumSongs(id, title, artist)
        }
        return albumOne
    }
}